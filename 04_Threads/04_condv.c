...
void *watch_count(void *t){
	...
	/* Lock mutex and wait for signal. note that the pthread_cons_wait routine
	will automatically and atomically unlock mutex whilw it waits. Also, note
	that if COUNT_LIMIT is reached before this routine is run by the waiting
	thread, the loop will be skipped to prevent pthread_cond_wait from never 
	returning */
