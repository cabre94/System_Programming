...
void* thread_StageB(void* arg){
	...
	while(tries != MAX_TRIES){
		...
		while(sync->idx_control != request.id){	// Espero a que el indice coincida con el id del request
			pthread_cond_wait(&sync->new_input_flag, &sync->mtx_out);
        }

		// ya puedo poner en la cola de la salida el request
		QueuePut(sync->out, &request);
		sync->idx_control++;

		if(pthread_cond_signal(&sync->new_input_flag) != 0){
...
int main(){
	...
	// queues_t stageB_queues = {&q_AB, &q_BC};
	// if(pthread_create(&stageB[0], &attr, thread_StageA, (void*) &stageB_queues) < 0)
	// 	exit(1);
